...
myInput = "1231240383"
inputArray = myInput.split(" ")
print(len(inputArray))
print(inputArray)
workingArray = []

f = open("stldailydata.txt", "w")
f.write("")
f.close()

myclient = SmiteClient(4174, "1B7B31B5396B428AB6517860ED7049A5")
signature = (myclient._create_signature("createsession"))
response = requests.get("https://api.smitegame.com/smiteapi.svc/createsessionxml/4174/" + signature + "/" + myclient._create_now_timestamp())
print(response.text)
mySession = response.text.split("session_id")
mySession = mySession[1][1:-2] 

i = 0
for x in inputArray:
    i += 1
    workingArray.append(x)
    if(i % 5 == 0):
        print(workingArray)
        signature2 = (myclient._create_signature("getmatchdetailsbatch"))
        response2 = requests.get("https://api.smitegame.com/smiteapi.svc/getmatchdetailsbatchxml/4174/" + signature2 + "/" + mySession + "/" + myclient._create_now_timestamp() + "/" + workingArray[0] + "," + workingArray[1] + "," + workingArray[2] + "," + workingArray[3] + "," + workingArray[4])
        f = open("stldailydata.txt", "a")
        f.write(response2.text.encode(sys.stdout.encoding, errors='replace').decode(sys.stdout.encoding) )
        f.close()
        workingArray = []
if(not(i % 5 == 0)):
    print(workingArray)
    matchIDs = ""
    for y in workingArray:
        matchIDs = matchIDs + y + ","
    matchIDs = matchIDs[0:-1]
    signature2 = (myclient._create_signature("getmatchdetailsbatch"))
    response2 = requests.get("https://api.smitegame.com/smiteapi.svc/getmatchdetailsbatchxml/4174/" + signature2 + "/" + mySession + "/" + myclient._create_now_timestamp() + "/" + matchIDs)
    f = open("stldailydata.txt", "a")
    f.write(response2.text.encode(sys.stdout.encoding, errors='replace').decode(sys.stdout.encoding) )
    f.close()
    workingArray = []

#signature3 = (myclient._create_signature("getitems"))
#response3 = requests.get("https://api.smitegame.com/smiteapi.svc/getitemsxml/4174/" + signature3 + "/" + mySession + "/" + myclient._create_now_timestamp() + "/" + "1")
#f = open("stldailydata.txt", "a")
#f.write(response3.text.encode(sys.stdout.encoding, errors='replace').decode(sys.stdout.encoding) )
#f.close()
...
#mySession = mySession[1][1:-2] 
#print(mySession)
#signature2 = (myclient._create_signature("getmatchdetails"))
#response2 = requests.get("https://api.smitegame.com/smiteapi.svc/getmatchdetailsxml/4174/" + signature2 + "/" + mySession + "/" + myclient._create_now_timestamp() + "/" + "1211977359")
